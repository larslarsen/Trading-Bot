"""Regime fallback: d40 trending -> REI choppy."""
import sys
...
def rei_signals(df):
    ...
    entry = pd.Series(((rei > 0) & (rei.diff() > 0)).astype(int), index=df.index)
    exit_sig = pd.Series((rei < -20).astype(int), index=df.index)
    return entry, exit_sig


print('Loading...', flush=True)
coin_data = load_screened_universe()
...
print('\n=== rei baseline (full) ===', flush=True)
res_i = simulate_portfolio(price_df, sig_entry, initial=INITIAL, max_positions=MAX_POSITIONS, exit_signal_df=sig_exit)
print(f"rei: ret={res_i['return_pct']:.1f}% sharpe={res_i['sharpe']:.2f} dd={res_i['max_dd_pct']:.1f}% trades={res_i['trades']}", flush=True)

print('\n=== regime fallback d40->rei (full) ===', flush=True)
res_reg = run_regime(full_dates, price_df, sig_d40, sig_entry, sig_exit, realized_vol)
print(f"regime_d40_rei full: ret={res_reg['return_pct']:.1f}% sharpe={res_reg['sharpe']:.2f} dd={res_reg['max_dd_pct']:.1f}% trades={res_reg['trades']}", flush=True)

print('\n=== regime fallback d40->rei (90d) ===', flush=True)
...
print(f"regime_d40_rei 90d: ret={res_reg90['return_pct']:.1f}% sharpe={res_reg90['sharpe']:.2f} dd={res_reg90['max_dd_pct']:.1f}% trades={res_reg90['trades']}", flush=True)
