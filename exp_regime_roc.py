"""Regime fallback: d40 trending -> ROC choppy."""
import sys
...
def roc_signals(df):
    ...
    roc = pd.Series(close, index=df.index).pct_change(10) * 100
    entry = (roc > 5).astype(int)
    exit_sig = (roc < -5).astype(int)
...
print('\n=== regime fallback d40->roc (full) ===', flush=True)
res_reg = run_regime(full_dates, price_df, sig_d40, sig_entry, sig_exit, realized_vol)
print(f"regime_d40_roc full: ret={res_reg['return_pct']:.1f}% sharpe={res_reg['sharpe']:.2f} dd={res_reg['max_dd_pct']:.1f}% trades={res_reg['trades']}", flush=True)

print('\n=== regime fallback d40->roc (90d) ===', flush=True)
...
print(f"regime_d40_roc 90d: ret={res_reg90['return_pct']:.1f}% sharpe={res_reg90['sharpe']:.2f} dd={res_reg90['max_dd_pct']:.1f}% trades={res_reg90['trades']}", flush=True)
