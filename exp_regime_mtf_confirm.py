"""Regime fallback: d40 trending -> MTF confirm choppy.
Tuning knobs at top.
"""
import sys
...
def mtf_signals(df):
    ...
    entry = pd.Series(((sma50 > sma200) & (close > sma50)).astype(int), index=df.index)
    exit_sig = pd.Series((close < sma50).astype(int), index=df.index)
...
sig_mtf_entry = pd.DataFrame(index=all_dates, columns=list(coin_data.keys()), dtype=int)
sig_mtf_exit = pd.DataFrame(index=all_dates, columns=list(coin_data.keys()), dtype=int)
...
for stem, df in coin_data.items():
    ...
    for i in range(len(df)):
        if mask.iloc[i]:
            ...
            sig_mtf_entry.loc[ts, stem] = int(e.iloc[i]) if pd.notna(e.iloc[i]) else 0
            sig_mtf_exit.loc[ts, stem] = int(x.iloc[i]) if pd.notna(x.iloc[i]) else 0

price_df = price_df.sort_index()
sig_d40 = sig_d40.sort_index()
sig_mtf_entry = sig_mtf_entry.sort_index()
sig_mtf_exit = sig_mtf_exit.sort_index()
...
print('\n=== mtf_confirm baseline (full) ===', flush=True)
res_i = simulate_portfolio(price_df, sig_mtf_entry, initial=INITIAL, max_positions=MAX_POSITIONS, exit_signal_df=sig_mtf_exit)
print(f"mtf_confirm: ret={res_i['return_pct']:.1f}% sharpe={res_i['sharpe']:.2f} dd={res_i['max_dd_pct']:.1f}% trades={res_i['trades']}", flush=True)

print('\n=== regime fallback d40->mtf (full) ===', flush=True)
res_reg = run_regime(full_dates, price_df, sig_d40, sig_mtf_entry, sig_mtf_exit, realized_vol)
print(f"regime_d40_mtf full: ret={res_reg['return_pct']:.1f}% sharpe={res_reg['sharpe']:.2f} dd={res_reg['max_dd_pct']:.1f}% trades={res_reg['trades']}", flush=True)

print('\n=== regime fallback d40->mtf (90d) ===', flush=True)
res_reg90 = run_regime(recent_dates, price_df.loc[recent_dates], sig_d40.loc[recent_dates], sig_mtf_entry.loc[recent_dates], sig_mtf_exit.loc[recent_dates], realized_vol.loc[recent_dates])
print(f"regime_d40_mtf 90d: ret={res_reg90['return_pct']:.1f}% sharpe={res_reg90['sharpe']:.2f} dd={res_reg90['max_dd_pct']:.1f}% trades={res_reg90['trades']}", flush=True)
