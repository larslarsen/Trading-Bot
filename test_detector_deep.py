"""
Regime DETECTOR comparison on the DEEP universe (66 WF slices, 9 majors/mids).
Run the SAME full strategy (REI trend + ATR; donchian40 + ma30_ema chop fill) but
swap ONLY the regime detector method. Tests whether a better detector exists now that
we have statistical power. Detectors: rule (current ADX+ER+vol), ma (50/200),
choppiness (CI>61.8), kaufman (ER-only), mesa (adaptive ER).

Panel + paired exact sign test vs current 'rule' detector.
"""
...
def main():
    ...
    print(f"DEEP detector comparison: {len(DETECTORS)} detectors, {len(slices)} WF slices, {len(data)} deep majors\n")
    res = {m: [run(data, seg, m) for seg in slices] for m in DETECTORS}
    print(f"{'detector':12s} {'meanRet':>8} {'effSR':>7} {'meanDD':>7} {'Calmar':>7}")
    agg = {}
    for m in DETECTORS:
        rs = res[m]
        agg[m] = (np.mean([r[0] for r in rs]), np.mean([r[1] for r in rs]),
                  np.mean([r[2] for r in rs]), np.mean([r[3] for r in rs]))
    for m in sorted(agg, key=lambda x: -agg[x][0]):
        a = agg[m]
        print(f"{m:12s} {a[0]:+8.1f} {a[1]:+7.2f} {a[2]:>6.1f}% {a[3]:>7.2f}")
    print(f"\nPaired vs current 'rule' detector (exact sign test, {len(slices)} slices):")
    ...
    for m in DETECTORS:
        ...
        print(f"   {m:12s} Δmean={np.mean(other)-np.mean(base):+6.1f}  beats in {k}/{n}  p={p:.3f}{' *' if p<0.05 else ''}")
