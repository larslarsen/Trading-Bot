"""Regime fallback: d40 trending -> BOP choppy."""
import sys
...
def bop_signals(df):
    ...
    high = df['high'].values
    low = df['low'].values
    bop = (close - low) / (high - low + 1e-12)
    sma_bop = pd.Series(bop, index=df.index).rolling(20).mean()
    entry = pd.Series(((bop > 0) & (sma_bop.diff() > 0)).astype(int), index=df.index)
    exit_sig = pd.Series((bop < -0.2).astype(int), index=df.index)
...
print('\n=== regime fallback d40->bop (full) ===', flush=True)
res_reg = run_regime(full_dates, price_df, sig_d40, sig_entry, sig_exit, realized_vol)
print(f"regime_d40_bop full: ret={res_reg['return_pct']:.1f}% sharpe={res_reg['sharpe']:.2f} dd={res_reg['max_dd_pct']:.1f}% trades={res_reg['trades']}", flush=True)

print('\n=== regime fallback d40->bop (90d) ===', flush=True)
...
print(f"regime_d40_bop 90d: ret={res_reg90['return_pct']:.1f}% sharpe={res_reg90['sharpe']:.2f} dd={res_reg90['max_dd_pct']:.1f}% trades={res_reg90['trades']}", flush=True)
