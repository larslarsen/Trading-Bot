"""Regime fallback: d40 trending -> Chandelier Exit choppy."""
import sys
...
def chandelier_signals(df):
    close = df['close'].values
    high = df['high'].values
    low = df['low'].values
    atr = pd.Series(close, index=df.index).ewm(span=10, adjust=False).mean().ewm(span=10, adjust=False).mean().ewm(span=10, adjust=False).mean()
    highest = pd.Series(high, index=df.index).rolling(22, min_periods=1).max()
    ce = highest - 3 * atr
    trend = (pd.Series(close, index=df.index) > ce).astype(int)
    entry = trend.diff().fillna(0).astype(int)
    entry = entry.where(entry == 1, 0).astype(int)
    exit_sig = trend.diff().fillna(0).astype(int)
    exit_sig = exit_sig.where(exit_sig == -1, 0).abs().astype(int)
...
for stem, df in coin_data.items():
    ...
    for i in range(len(df)):
        if mask.iloc[i]:
            ...
            sig_entry.loc[ts, stem] = int(e.iloc[i]) if pd.notna(e.iloc[i]) else 0
            sig_exit.loc[ts, stem] = int(x.iloc[i]) if pd.notna(x.iloc[i]) else 0
...
print('\n=== chandelier_exit baseline (full) ===', flush=True)
res_i = simulate_portfolio(price_df, sig_entry, initial=INITIAL, max_positions=MAX_POSITIONS, exit_signal_df=sig_exit)
print(f"chandelier_exit: ret={res_i['return_pct']:.1f}% sharpe={res_i['sharpe']:.2f} dd={res_i['max_dd_pct']:.1f}% trades={res_i['trades']}", flush=True)

print('\n=== regime fallback d40->chandelier_exit (full) ===', flush=True)
res_reg = run_regime(full_dates, price_df, sig_d40, sig_entry, sig_exit, realized_vol)
print(f"regime_d40_chandelier_exit full: ret={res_reg['return_pct']:.1f}% sharpe={res_reg['sharpe']:.2f} dd={res_reg['max_dd_pct']:.1f}% trades={res_reg['trades']}", flush=True)

print('\n=== regime fallback d40->chandelier_exit (90d) ===', flush=True)
res_reg90 = run_regime(recent_dates, price_df.loc[recent_dates], sig_d40.loc[recent_dates], sig_entry.loc[recent_dates], sig_exit.loc[recent_dates], realized_vol.loc[recent_dates])
print(f"regime_d40_chandelier_exit 90d: ret={res_reg90['return_pct']:.1f}% sharpe={res_reg90['sharpe']:.2f} dd={res_reg90['max_dd_pct']:.1f}% trades={res_reg90['trades']}", flush=True)
