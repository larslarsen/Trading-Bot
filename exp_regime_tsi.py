"""Regime fallback: d40 trending -> TSI choppy."""
import sys
...
def tsi_signals(df):
    ...
    price = pd.Series(close, index=df.index)
    momentum = price.diff()
    ema1 = momentum.ewm(span=13, adjust=False).mean()
    ema2 = ema1.ewm(span=13, adjust=False).mean()
    abs1 = momentum.abs().ewm(span=13, adjust=False).mean()
    abs2 = abs1.ewm(span=13, adjust=False).mean()
...
print("d40 baseline (full)", flush=True)
res = simulate_portfolio(price_df, sig_d40, initial=INITIAL, max_positions=MAX_POSITIONS)
print("baseline_d40: ret=" + str(round(res["return_pct"], 1)) + "% sharpe=" + str(round(res["sharpe"], 2)) + " dd=" + str(round(res["max_dd_pct"], 1)) + "% trades=" + str(res["trades"]), flush=True)

print("tsi baseline (full)", flush=True)
res_i = simulate_portfolio(price_df, sig_entry, initial=INITIAL, max_positions=MAX_POSITIONS, exit_signal_df=sig_exit)
print("tsi: ret=" + str(round(res_i["return_pct"], 1)) + "% sharpe=" + str(round(res_i["sharpe"], 2)) + " dd=" + str(round(res_i["max_dd_pct"], 1)) + "% trades=" + str(res_i["trades"]), flush=True)

print("regime fallback d40->tsi (full)", flush=True)
res_reg = run_regime(full_dates, price_df, sig_d40, sig_entry, sig_exit, realized_vol)
print("regime_d40_tsi full: ret=" + str(round(res_reg["return_pct"], 1)) + "% sharpe=" + str(round(res_reg["sharpe"], 2)) + " dd=" + str(round(res_reg["max_dd_pct"], 1)) + "% trades=" + str(res_reg["trades"]), flush=True)

print("regime fallback d40->tsi (90d)", flush=True)
...
print("regime_d40_tsi 90d: ret=" + str(round(res_reg90["return_pct"], 1)) + "% sharpe=" + str(round(res_reg90["sharpe"], 2)) + " dd=" + str(round(res_reg90["max_dd_pct"], 1)) + "% trades=" + str(res_reg90["trades"]), flush=True)
