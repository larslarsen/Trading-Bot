"""
(b) SIZING SWEEP on DEEP universe (66 WF slices, 9 majors) — closes the loop:
the original sizing sweep ran on the 8-slice low-cap universe; re-run here with
real statistical power. Per Scholz 2012: smaller fractions => better risk-adjusted
in MOST scenarios, no optimal fraction exists. Test LIVE full system across
{10,15,20,25}% position fractions. Panel + paired sign test vs current 20%.
"""
...
def run(data, dates, frac):
    cfg = EngineConfig(
        initial_capital=10000.0, max_daily_loss_pct=0.03, max_drawdown_pct=0.20,
        max_positions=MAX_POS, max_position_pct=frac, min_equity_to_trade=100.0,
    ...
    for day in dates:
        ...
        for s in active:
            ...
            ok, _ = eng.check_circuit_breakers()
            if not ok: break
            eng.open_position(s, px, eng.equity * frac)
...
def main():
    ...
    print(f"(b) SIZING SWEEP on DEEP: LIVE full system, {len(slices)} WF slices, {len(data)} majors\n")
    res = {f: [run(data, seg, f) for seg in slices] for f in FRACTIONS}
    print(f"{'frac':>5} {'meanRet':>8} {'effSR':>7} {'meanDD':>7} {'Calmar':>7}")
    agg = {}
    for f in FRACTIONS:
        rs = res[f]
        agg[f] = (np.mean([r[0] for r in rs]), np.mean([r[1] for r in rs]),
                  np.mean([r[2] for r in rs]), np.mean([r[3] for r in rs]))
    for f in FRACTIONS:
        a = agg[f]
        print(f"{int(f*100):>4}% {a[0]:+8.1f} {a[1]:+7.2f} {a[2]:>6.1f}% {a[3]:>7.2f}")
    print("\nPaired vs current 20% (exact sign test on slice returns):")
    base = [r[0] for r in res[0.20]]
    for f in FRACTIONS:
        if f == 0.20: continue
        other = [r[0] for r in res[f]]
        ...
        print(f"   {int(f*100):>3}% vs 20%: Δmean={np.mean(other)-np.mean(base):+6.1f}  beats in {k}/{n}  p={p:.3f}{' *' if p<0.05 else ''}")
    best_sr = max(FRACTIONS, key=lambda f: agg[f][1])
    best_cal = max(FRACTIONS, key=lambda f: agg[f][3])
    print(f"\n   Best effSR at {int(best_sr*100)}% ({agg[best_sr][1]:+.2f}); best Calmar at {int(best_cal*100)}% ({agg[best_cal][3]:.2f})")
    print("   Scholz 2012 predicts smaller => better risk-adjusted; check if 10/15% win on effSR/Calmar.")
