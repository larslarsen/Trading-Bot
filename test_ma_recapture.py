"""
MA-RECAPTURE family, isolated. Run ONLY the MA recapture variants as the chop leg
(trend=REI + ATR) on the deep 66-slice majors universe. Variants:
  ma30        - close > SMA30 (basic recapture)
  ma30_ema    - close > EMA30 (live fill-in rule)
  ma30_rising - EMA30 slope > 0
  ma30_50     - close > SMA30 AND SMA30 > SMA50
  d40+ma30_ema - donchian40 primary + ma30_ema fill (LIVE combo, for reference)
Plus donchian40 as the baseline chop leg.

Panel + paired exact sign test, 66 WF slices, 9 deep majors.
"""
...
MIN_BARS = 800
CANDIDATES = ["ma30", "ma30_ema", "ma30_rising", "ma30_50",
              "donchian40", ("d40+ma30_ema", "combo")]
...
def main():
    ...
    print(f"MA-RECAPTURE family (isolated): {len(CANDIDATES)} variants, {len(slices)} WF slices, {len(data)} deep majors\n")
    res = {}
    for c in CANDIDATES:
    ...
    print(f"{'variant':14s} {'meanRet':>8} {'worst':>7} {'pos/n':>6} {'effSR':>7} {'meanDD':>7} {'Calmar':>7} {'win%':>6}")
    agg = {}
    for name in res:
        rs = res[name]
        if not rs:
    ...
    print(f"\nPaired vs donchian40 (exact sign test, {len(slices)} slices):")
    ...
    # paired MA variants vs ma30_ema (live fill-in)
    print(f"\nPaired vs ma30_ema (live fill-in rule):")
    base2 = [r[0] for r in res["ma30_ema"]]
    for name in res:
        if name in ("ma30_ema", "donchian40"): continue
        other = [r[0] for r in res[name]]
        d = np.array(other) - np.array(base2)
        n = np.sum(d != 0); k = np.sum(d > 0)
        from math import comb
        p = min(1.0, 2 * sum(comb(n, i) for i in range(min(k, n - k) + 1)) / 2 ** n) if n else 1.0
        print(f"   {name:14s} Δmean={np.mean(other)-np.mean(base2):+6.1f}  beats in {k}/{n}  p={p:.3f}{' *' if p<0.05 else ''}")
