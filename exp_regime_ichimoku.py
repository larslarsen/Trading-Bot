"""Regime fallback: d40 trending -> Ichimoku choppy.
Tuning knobs at top.
"""
import sys
...
def ichimoku_signals(df):
    ...
    high = df['high'].values
    low = df['low'].values
    tenkan = (pd.Series(high).rolling(9, min_periods=1).max() + pd.Series(low).rolling(9, min_periods=1).min()) / 2
    kijun = (pd.Series(high).rolling(26, min_periods=1).max() + pd.Series(low).rolling(26, min_periods=1).min()) / 2
    span_a = (tenkan + kijun) / 2
    span_b = (pd.Series(high).rolling(52, min_periods=1).max() + pd.Series(low).rolling(52, min_periods=1).min()) / 2
    entry = ((tenkan > kijun) & (pd.Series(close) > span_a) & (pd.Series(close) > span_b)).astype(int)
    exit_sig = (pd.Series(close) < kijun).astype(int)
...
sig_ichi_entry = pd.DataFrame(index=all_dates, columns=list(coin_data.keys()), dtype=int)
sig_ichi_exit = pd.DataFrame(index=all_dates, columns=list(coin_data.keys()), dtype=int)

for stem, df in coin_data.items():
    close = df['close'].values
    e, x = ichimoku_signals(df)
    ...
    for i in range(len(df)):
        if mask.iloc[i]:
            ...
            sig_ichi_entry.loc[ts, stem] = int(e.iloc[i]) if pd.notna(e.iloc[i]) else 0
            sig_ichi_exit.loc[ts, stem] = int(x.iloc[i]) if pd.notna(x.iloc[i]) else 0

price_df = price_df.sort_index()
sig_d40 = sig_d40.sort_index()
sig_ichi_entry = sig_ichi_entry.sort_index()
sig_ichi_exit = sig_ichi_exit.sort_index()
...
print('\n=== ichimoku baseline (full) ===', flush=True)
res_i = simulate_portfolio(price_df, sig_ichi_entry, initial=INITIAL, max_positions=MAX_POSITIONS, exit_signal_df=sig_ichi_exit)
print(f"ichimoku: ret={res_i['return_pct']:.1f}% sharpe={res_i['sharpe']:.2f} dd={res_i['max_dd_pct']:.1f}% trades={res_i['trades']}", flush=True)

print('\n=== regime fallback d40->ichimoku (full) ===', flush=True)
res_reg = run_regime(full_dates, price_df, sig_d40, sig_ichi_entry, sig_ichi_exit, realized_vol)
print(f"regime_d40_ichi full: ret={res_reg['return_pct']:.1f}% sharpe={res_reg['sharpe']:.2f} dd={res_reg['max_dd_pct']:.1f}% trades={res_reg['trades']}", flush=True)

print('\n=== regime fallback d40->ichimoku (90d) ===', flush=True)
res_reg90 = run_regime(recent_dates, price_df.loc[recent_dates], sig_d40.loc[recent_dates], sig_ichi_entry.loc[recent_dates], sig_ichi_exit.loc[recent_dates], realized_vol.loc[recent_dates])
print(f"regime_d40_ichi 90d: ret={res_reg90['return_pct']:.1f}% sharpe={res_reg90['sharpe']:.2f} dd={res_reg90['max_dd_pct']:.1f}% trades={res_reg90['trades']}", flush=True)
