...
"""Regime fallback: d40 trending -> OBV choppy."""
import sys
...
def obv_signals(df):
    ...
    for i in range(1, len(df)):
        if close[i] > close[i - 1]:
            obv.iloc[i] = obv.iloc[i - 1] + vol[i]
        elif close[i] < close[i - 1]:
            obv.iloc[i] = obv.iloc[i - 1] - vol[i]
        else:
            obv.iloc[i] = obv.iloc[i - 1]
    ma = obv.rolling(20, min_periods=1).mean()
    entry = ((obv > ma) & (obv.diff() > 0)).astype(int)
    exit_sig = ((obv < ma) & (obv.diff() < 0)).astype(int)
...
print("regime fallback d40->obv (full)", flush=True)
res_reg = run_regime(full_dates, price_df, sig_d40, sig_entry, sig_exit, realized_vol)
print("regime_d40_obv full: ret=" + str(round(res_reg["return_pct"], 1)) + "% sharpe=" + str(round(res_reg["sharpe"], 2)) + " dd=" + str(round(res_reg["max_dd_pct"], 1)) + "% trades=" + str(res_reg["trades"]), flush=True)

print("regime fallback d40->obv (90d)", flush=True)
...
print("regime_d40_obv 90d: ret=" + str(round(res_reg90["return_pct"], 1)) + "% sharpe=" + str(round(res_reg90["sharpe"], 2)) + " dd=" + str(round(res_reg90["max_dd_pct"], 1)) + "% trades=" + str(res_reg90["trades"]), flush=True)
