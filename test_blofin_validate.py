"""
BLOFIN validation: validate the live config on a SECOND live-trading venue
(derivatives/perp alt venue, matches the user's actual book — unlike Kraken spot).
495 BloFin pairs backfilled; 175 with >=800 bars share an 806-bar window
(2024-04-30 -> 2026-07-14) => ~40 WF slices. Tests whether the deep-universe
conclusions (donchian40 chop solid, tsi/bop not better, 'rule' detector best)
GENERALIZE to a derivatives venue.

Candidates: chop rule {donchian40, tsi, bop, ma30_ema} + detector {rule, ma, bbwp}.
"""
...
DETECTORS = ["rule", "ma", "bbwp"]


def load_blofin(n_min=MIN_BARS):
    from pathlib import Path
    files = sorted(Path('data').glob('*_blofin_1d_max.csv'))
    data = {}
    for p in files:
        try:
            df = pd.read_csv(p)
            ...
            data[p.name.replace('_blofin_1d_max.csv', '')] = df.set_index('ts')
        except Exception:
            continue
    idx = None
...
def main():
    data, dates = load_blofin()
    oos, step, warmup = 20, 20, 120
    slices = []
    i = warmup
    while i + oos <= len(dates):
        slices.append(dates[i:i + oos]); i += step
    print(f"BLOFIN validation: {len(CHOP)} chop x {len(DETECTORS)} det, {len(slices)} WF slices, {len(data)} coins\n")
    res = {}
    for m in DETECTORS:
        for c in CHOP:
            key = f"{c}|{m}"
