"""Regime fallback: d40 trending -> SuperTrend choppy."""
import sys
...
def supertrend_signals(df):
    ...
    entry = ((trend == 1.0) & (trend.diff() > 0)).astype(int)
    exit_sig = ((trend == -1.0) & (trend.diff() < 0)).astype(int)
...
print('\n=== supertrend baseline (full) ===', flush=True)
res_i = simulate_portfolio(price_df, sig_entry, initial=INITIAL, max_positions=MAX_POSITIONS, exit_signal_df=sig_exit)
print(f"supertrend: ret={res_i['return_pct']:.1f}% sharpe={res_i['sharpe']:.2f} dd={res_i['max_dd_pct']:.1f}% trades={res_i['trades']}", flush=True)

print('\n=== regime fallback d40->supertrend (full) ===', flush=True)
res_reg = run_regime(full_dates, price_df, sig_d40, sig_entry, sig_exit, realized_vol)
print(f"regime_d40_supertrend full: ret={res_reg['return_pct']:.1f}% sharpe={res_reg['sharpe']:.2f} dd={res_reg['max_dd_pct']:.1f}% trades={res_reg['trades']}", flush=True)

print('\n=== regime fallback d40->supertrend (90d) ===', flush=True)
...
print(f"regime_d40_supertrend 90d: ret={res_reg90['return_pct']:.1f}% sharpe={res_reg90['sharpe']:.2f} dd={res_reg90['max_dd_pct']:.1f}% trades={res_reg90['trades']}", flush=True)
